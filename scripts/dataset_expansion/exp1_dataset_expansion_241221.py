...
def generate_data(args):
    ...
    for key in tqdm(subsets.keys()):
        ...
        if filename in os.listdir(generated_data_directory): # move to next category if there's already an associated file of generations in the directory
            ...
            subset_df = subsets[key]

            if len(subset_df) < n_sampled:
                print(f"Subset {keystring} has only {len(subset_df)} rows, using all available rows.")
                sample = subset_df  # Use all rows if not enough to sample
            else:
                sample = subset_df.sample(n_sampled, random_state=random.sample(range(0, 1000), 1)[0])
